...
export function MemoryRunner({ config, mode, onComplete }: MemoryRunnerProps) {
  ...
  const handleCardClick = (index: number) => {
    if (gameState !== 'playing' || isLocking || cards[index].isFlipped || cards[index].isMatched) {
      return;
    }
    ...
    if (newFlipped.length === 2) {
      ...
      const match = newCards[firstIndex].pairId === newCards[secondIndex].pairId;
      ...
      setTimeout(() => {
        ...
        if (match) {
          ...
          if (finalCards.every((c) => c.isMatched)) {
            setGameState('result');
            if (onComplete) {
              onComplete({ moves: moves + 1, time: timer });
            }
          }
        } else {
      ...
      <div className="min-h-[500px] bg-white flex flex-col items-center justify-center p-8 text-center animate-fade-in rounded-2xl shadow-xl">
        ...
        <div className="flex gap-8 mb-8 text-slate-600 font-bold">
          <div className="text-center">
            <p className="text-xs uppercase opacity-50">Tempo</p>
            <p className="text-2xl font-mono">{timer}s</p>
          </div>
          <div className="text-center">
            <p className="text-xs uppercase opacity-50">Moves</p>
            <p className="text-2xl font-mono">{moves}</p>
          </div>
        </div>

        <button
          onClick={() => window.location.reload()}
          className="px-10 py-4 bg-indigo-600 text-white rounded-xl font-bold flex items-center gap-2 hover:bg-indigo-700 transition-all shadow-lg"
        >
  ...
  return (
    <div className="w-full flex flex-col bg-slate-950 overflow-hidden relative rounded-2xl shadow-2xl border border-white/5">
      <div className="flex justify-between items-center p-4 bg-black/40 border-b border-white/10 z-10 backdrop-blur-md">
        <span className="text-white/70 text-xs font-bold uppercase tracking-widest truncate max-w-[200px]">
          {config.title}
        </span>

        <div className="flex gap-4">
          <div className="flex items-center gap-2 text-cyan-400 font-mono text-sm bg-black/50 px-3 py-1 rounded-full">
            <Clock size={14} /> {timer}s
          </div>
          <div className="flex items-center gap-2 text-purple-400 font-mono text-sm bg-black/50 px-3 py-1 rounded-full">
            ...
            >
              <div
                className={`w-full h-full transition-all duration-500 transform-style-3d cursor-pointer ${
                  card.isFlipped || card.isMatched ? 'rotate-y-180' : 'group-hover:scale-105'
                }`}
              >
                ...
                <div
                  className={`absolute inset-0 bg-white rounded-xl backface-hidden rotate-y-180 overflow-hidden border-2 ${
                    card.isMatched ? 'border-green-500 shadow-lg' : 'border-white'
                  }`}
                >
                  <img
                    src={card.content}
                    className={`w-full h-full object-cover ${card.isMatched ? 'opacity-40' : ''}`}
                    alt="Memory Card"
                  />
                </div>
              </div>
