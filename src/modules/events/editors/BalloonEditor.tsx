...
// Trash2 removido da linha abaixo para eliminar o erro
import { Save, ArrowLeft, Upload, Lock, CheckCircle, Loader2, Image as ImageIcon, Settings, Palette, Clock, Gauge, GripHorizontal, Eye } from 'lucide-react';
      ...
      <div className="max-w-5xl mx-auto mt-8 px-6 space-y-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div onClick={() => logoInputRef.current?.click()} className="aspect-video rounded-xl border-2 border-dashed border-slate-300 hover:border-purple-500 cursor-pointer flex flex-col items-center justify-center overflow-hidden transition-all group bg-slate-50">
                {config.logoUrl ? <img src={config.logoUrl} className="h-full object-contain" alt="Logo" /> : <><Upload className="text-slate-400 group-hover:text-purple-500" /><span className="text-xs font-bold text-slate-500">Logo Evento</span></>}
                <input type="file" className="hidden" ref={logoInputRef} onChange={e => handleUpload('logoUrl', e)} accept="image/*" />
            </div>
            <div onClick={() => bgInputRef.current?.click()} className="aspect-video rounded-xl border-2 border-dashed border-slate-300 hover:border-purple-500 cursor-pointer flex flex-col items-center justify-center overflow-hidden transition-all group bg-slate-50">
                {config.backgroundImageUrl ? <img src={config.backgroundImageUrl} className="w-full h-full object-cover" alt="Fundo" /> : <><ImageIcon className="text-slate-400 group-hover:text-purple-500" /><span className="text-xs font-bold text-slate-500">Imagem Fundo</span></>}
                <input type="file" className="hidden" ref={bgInputRef} onChange={e => handleUpload('backgroundImageUrl', e)} accept="image/*" />
            </div>
            <div onClick={() => balloonLogoInputRef.current?.click()} className="aspect-video rounded-xl border-2 border-dashed border-slate-300 hover:border-purple-500 cursor-pointer flex flex-col items-center justify-center overflow-hidden transition-all group bg-slate-50">
                {config.balloonLogoUrl ? (
                  <div className="relative w-16 h-20 bg-red-500 rounded-[50%] flex items-center justify-center shadow-lg">
                    <img src={config.balloonLogoUrl} className="w-8 h-8 object-contain opacity-80" alt="Logo Balão" />
                  </div>
                ) : (
                  <><div className="w-8 h-10 bg-slate-200 rounded-full mb-2"></div><span className="text-xs font-bold text-slate-500">Logo Balão</span></>
                )}
              ...
              return (
                <button key={color.id} onClick={() => toggleColor(color.hex)} className={`flex flex-col items-center gap-3 p-4 rounded-xl border-2 transition-all w-28 ${isActive ? 'border-purple-500 bg-purple-50 shadow-md' : 'border-slate-100 bg-white opacity-60 grayscale'}`}>
                  <div className="w-10 h-12 rounded-[50%] shadow-sm relative" style={{ backgroundColor: color.hex }}>
                      ...
                      {config.balloonLogoUrl && <img src={config.balloonLogoUrl} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-5 h-5 object-contain opacity-70" alt="" />}
                  </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex justify-between items-center">
            ...
            <button onClick={() => setConfig({...config, skipLeadGate: !config.skipLeadGate})} className={`px-4 py-2 rounded-lg font-bold text-sm transition-colors ${!config.skipLeadGate ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>{!config.skipLeadGate ? 'ATIVADO' : 'DESATIVADO'}</button>
        </div>
